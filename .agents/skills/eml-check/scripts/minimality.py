...
def _resolve_target(args) -> tuple[str, tuple[complex, ...], bool, list[complex], list[complex]]:
    """Returns (label, target_vec, binary_flag, xs, ys) or raises SystemExit."""
    ...
    if args.target:
        ...
        return args.target, vec, binary, xs, ref_ys
    if args.tree:
        try:
            ast = parse(args.tree)
        ...
        if vec is None:
            ...
            sys.exit(EXIT_USAGE)
        return f"tree: {args.tree}", vec, True, xs, ys
...
def cmd_audit_minimality(args) -> int:
    label, target_vec, binary, xs, ys = _resolve_target(args)
