...
def _cmd_search(args) -> int:
    ...
    try:
        result = beam_search(
            ...
            retain_k=retain_k,
            emit_variants=args.emit_variants,
        )
    except ValueError as e:
        print(f"error: {e}", file=sys.stderr)
        return EXIT_USAGE
    ...
    out = {
        ...
        "variants": {
            "emit_variants": args.emit_variants,
            "buckets_by_k": {
                K: len(result.variants_by_k[K])
                for K in sorted(result.variants_by_k)
            },
            "extras_by_k": {
                K: sum(len(v) for v in result.variants_by_k[K].values())
                for K in sorted(result.variants_by_k)
            },
        } if args.emit_variants > 1 else None,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    ps.add_argument(
        ...
        type=int,
        default=1,
        help="keep up to N K-equal syntactic siblings per hash bucket (default 1 = current behavior). "
        "Extras are stored in variants_by_k for seeding future beam runs; does not affect search result.",
    )
