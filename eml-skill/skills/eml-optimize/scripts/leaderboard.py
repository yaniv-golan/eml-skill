...
def _fmt_paper_k(w: Witness) -> str:
    """Render the paper-K cell with Table-4 provenance.

    Notation (P-paper-k-audit-2026-04-19):
      - `NNNᶜ`                — compiler-column scalar only
      - `NNNᵈ`                — direct-search column scalar only
      - `NNNᶜ / MMMᵈ`         — both columns published and non-trivially
                                 different (e.g. sqrt compiler=139, direct=43)
      - `NNNᶜ / >MMM`         — compiler published, direct-search timed out
                                 with lower-bound floor MMM (pi: 193 / >53)
      - `NNN?`                — scalar preserved but Table-4 provenance
                                 missing/unverifiable (paper_k_source=None)
      - `—`                   — no paper value in library

    The superscripts ᶜ and ᵈ are U+1D9C and U+1D48 (Unicode modifier letters
    small c / d) — they render in monospace columns and degrade to `c`/`d`
    in non-Unicode terminals.
    """
    if w.paper_k is None:
        return "—"
    if w.paper_k_source is None:
        # Scalar exists but Table-4 attribution is unverified (e.g. sin/cos/
        # tan/asin/acos/atan/log10 are not in Table 4 — values came from the
        # closure-proof-page and were mis-cited as Table 4).
        return f"{w.paper_k}?"
    if w.paper_k_source == "compiler":
        cell = f"{w.paper_k}ᶜ"
        # Augment with direct-search column when available.
        if w.paper_k_direct is not None:
            cell += f" / {w.paper_k_direct}ᵈ"
        elif w.paper_k_direct_lower is not None:
            cell += f" / >{w.paper_k_direct_lower}"
        return cell
    if w.paper_k_source == "direct-search":
        # Our scalar IS the direct-search value. Only annotate.
        return f"{w.paper_k}ᵈ"
    return str(w.paper_k)
...
def render_markdown() -> str:
    ...
    lines.append(
        ...
        "arXiv:2603.21852 Table 4, annotated with Table-4 column provenance: "
        "`NNNᶜ` = EML-compiler column (deterministic arithmetic artifact, "
        "Subsect 4.1); `NNNᵈ` = direct-search column (exhaustive-search K, "
        "the real research target); `NNNᶜ / MMMᵈ` = both columns published "
        "(e.g. sqrt compiler=139 vs direct=43); `NNNᶜ / >MMM` = direct search "
        "timed out with confirmed floor MMM; `NNN?` = scalar shipped but "
        "Table-4 provenance is unverifiable (see audit doc); `—` = no paper "
        "value. _proof-engine K_ = per-primitive value from a proof-engine "
        "page that publishes one (`—` for primitives that only appear on the "
        "calculator-closure apex proof without an individual K). _verdict_ = "
        "✅ minimal when exhaustively proven, 🔴 refuted-upward when a "
        "published paper K is not reproducible by this repo's search, 🟡 "
